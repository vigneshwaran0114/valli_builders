...
const ConstructionWorker = ({ position, isWorking = true }: ConstructionWorkerProps) => {
  ...
  useFrame((state) => {
    if (workerRef.current && isWorking) {
      // Worker breathing motion
      workerRef.current.position.y = position[1] + Math.sin(state.clock.elapsedTime * 2) * 0.02;
    }
    ...
    if (legRef.current && isWorking) {
      // Leg movement
      legRef.current.rotation.x = Math.sin(state.clock.elapsedTime * 2) * 0.2;
    }
  });
  ...
  return (
    <group ref={workerRef} position={position}>
      ...
      <mesh position={[0, 1.8, 0]}>
        ...
        <meshStandardMaterial color="#e53e3e" metalness={0.3} roughness={0.7} />
      </mesh>
